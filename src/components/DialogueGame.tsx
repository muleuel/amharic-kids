    ...
    return (
      <div className="mx-auto flex w-full max-w-2xl flex-1 flex-col gap-6 px-4 py-8">
        ...
        <div className="flex flex-1 flex-col justify-end gap-3">
          <div
            className={`bubble-card animate-pop flex max-w-[85%] flex-col gap-1 bg-white p-4 ${
              isLeft
                ? `self-start ${theme.border}`
                : "self-end border-purple-dark"
            }`}
          >
        ...
        <button
          onClick={() =>
            isLastLine ? setPhase("quiz") : setLineIndex((i) => i + 1)
          }
          className={`chunky-btn ${theme.bg} self-center px-8 py-3 text-lg text-white`}
        >
          {isLastLine ? "Start Questions →" : "Next →"}
        </button>
      </div>
    );
  }
  ...
  return (
    <div className="mx-auto flex w-full max-w-2xl flex-1 flex-col gap-6 px-4 py-8">
      ...
      <div className="bubble-card flex flex-1 flex-col items-center justify-center gap-8 border-white bg-white p-8">
        ...
        {selected && isLastQuestion && submitting && (
